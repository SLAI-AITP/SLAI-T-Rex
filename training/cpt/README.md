...
Shared conversion scripts live in [`../convert`](../convert/).
...
```bash
cd SLAI-T-Rex/training/convert

export MINDSPEED_LLM_DIR=/path/to/MindSpeed-LLM
export INPUT_FP8_HF_PATH=/path/to/deepseek4_fp8_hf
export OUTPUT_BF16_HF_PATH=/path/to/deepseek4_bf16_hf

bash convert_ckpt_fp8_to_bf16.sh
```

The output directory remains a HuggingFace checkpoint. Then convert BF16 HuggingFace to MindSpeed/Megatron-Core:
...
bash ../convert/convert_ckpt_hf_to_mcore.sh
```

This repository currently releases the checkpoint preparation script and training-side conversion hooks, not a full serving stack.
...
bash ../convert/convert_ckpt_mcore_to_hf.sh
```
