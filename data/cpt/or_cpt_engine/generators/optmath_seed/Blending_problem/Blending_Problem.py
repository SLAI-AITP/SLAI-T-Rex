...
class Generator:
    ...
    def generate_instance(self):
        ...
        percentages = {
            (e, a): uniform_rounded(*self.percentage_range)
            for e in elements
            for a in alloys
        }
        ...
        # Desired percentages are derived from a known feasible blend.
        # Independent element-wise sampling often falls outside the convex hull
        # of available alloy compositions and makes the LP infeasible.
        active_count = random.randint(2, min(4, self.n_alloys))
        active_alloys = random.sample(alloys, active_count)
        raw_weights = {a: random.random() + 0.1 for a in active_alloys}
        total_weight = sum(raw_weights.values())
        target_weights = {a: raw_weights.get(a, 0.0) / total_weight for a in alloys}
        desired_percentages = {}
        for i, e in enumerate(elements):
            if i < len(self.desired_percentages):
                desired_percentages[e] = round_business_float(self.desired_percentages[i])
            else:
                desired_percentages[e] = round_business_float(
                    sum(percentages[e, a] * target_weights[a] for a in alloys)
                )
        self.target_weights = target_weights
        self.desired_percentages_by_element = desired_percentages
        ...
        composition_tolerance = round_business_float(getattr(self, "composition_tolerance", 0.01))

        # Add tolerance-band constraints for desired element composition.
        # The targets are rounded for human-facing seed quality; a tight band
        # keeps the rounded specification feasible without reintroducing long
        # decimal equality constants.
        for e in elements:
            element_mix = gp.quicksum(percentages[e, a] * x[a] for a in alloys)
            model.addConstr(
                element_mix >= max(0.0, round_business_float(desired_percentages[e] - composition_tolerance)),
                name=f"Element_Min_{e}"
            )
            model.addConstr(
                element_mix <= min(1.0, round_business_float(desired_percentages[e] + composition_tolerance)),
                name=f"Element_Max_{e}"
            )
