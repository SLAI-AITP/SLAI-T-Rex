...
This internal version is a compact continuous mean-variance QP for CPT seed
generation. It emits `compact_portfolio_tables` containing assets, expected
returns, the full covariance matrix, target return, and weight bounds. These
tables are the preferred LLM-facing numeric source.

Do not reinterpret this generator as binary asset selection, knapsack,
project selection, transaction-cost rebalancing, or a cardinality-constrained
portfolio model.
