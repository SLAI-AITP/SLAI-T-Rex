...
This internal version is a sparse fixed-charge capacitated directed network-flow
model. It is not a multi-period inventory model, not node facility opening, and
not route sequencing.

The intended model is:

- `y[i,j]` is a binary activation decision for a declared directed arc.
- `x[i,j]` is nonnegative continuous flow on a declared directed arc.
- Objective is fixed arc activation cost plus unit arc flow cost.
- Capacity linking is `x[i,j] <= capacity[i,j] * y[i,j]`.
- Node balance is `inflow - outflow = demand - supply`.
- Only declared arcs may appear in variables, objective, and constraints.

The generator records a latent feasible flow only as a feasibility certificate;
it is not a required solution.
...
5. A sparse declared directed arc set is generated instead of a complete graph
6. Arc capacities are generated to ensure feasibility of the problem
7. Flow balance uses inflow minus outflow equals demand minus supply
