...
class Generator:
    def __init__(self, parameters=None, seed=None):
        ...
        default_parameters = {
            ...
            "food_amount_range": (0, 1.5)  # Range for food purchasing amounts
        }
    ...
    def generate_instance(self):
        ...
        target_amount = {food: 0.0 for food in foods}
        for food in target_foods:
            target_amount[food] = uniform_rounded(0.8, self.food_amount_range[1])
        min_nutrient = {}
        max_nutrient = {}
        for nutrient in nutrients:
            target_value = sum(nutrient_amount[(nutrient, food)] * target_amount[food] for food in foods)
            min_nutrient[nutrient] = max(0.01, round_business_float(target_value * 0.95))
            max_nutrient[nutrient] = max(
                round_business_float(min_nutrient[nutrient] + 0.01),
                round_business_float(target_value * 1.05),
            )
        ...
        self.min_nutrient = min_nutrient
        self.max_nutrient = max_nutrient
