...
class Generator:
    def __init__(self, parameters=None, seed=None):
        ...
        """
        default_parameters = {
            "n_nodes": (6, 10),  # Range for number of nodes
    ...
    def generate_instance(self):
        ...
        p_min, p_max = _as_range(self.p_facilities)
        self.p_facilities = random.randint(p_min, min(p_max, self.n_nodes - 2))

        # Generate nodes and distances
        nodes = [f"node_{i}" for i in range(self.n_nodes)]
        distances = {
            (nodes[left], nodes[right]): random.randint(*self.distance_range)
        ...
        self.parameters.update(
            {
                ...
                "distances": {f"{i}|{j}": value for (i, j), value in distances.items()},
            }
        )
