...
## OR-CPT Modeling Contract

This generator creates a multi-period production, sales, and inventory LP.

- Use continuous `Production[t,p]`, `Sales[t,p]`, and `Inventory[t,p]` variables.
- Preserve period-by-period inventory balance with zero initial inventory.
- Preserve machine-hour capacity for every period-machine pair after maintenance.
- Preserve product-period sales upper bounds; do not replace them with one inferred total demand value per product.
- Preserve the inventory upper bound and required final inventory equality for every product.
- Maximize sales revenue minus inventory holding cost.
- Use `compact_factory_planning_tables` as the preferred LLM-facing table bundle.
- Do not reinterpret this as a static product-mix model, public-program allocation, network expansion, or unconstrained production model.
