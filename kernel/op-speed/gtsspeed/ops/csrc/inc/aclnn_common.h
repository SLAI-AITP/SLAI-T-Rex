...
inline aclTensor *ConvertType(const at::Tensor &at_tensor)
{
    ...
    c10::SmallVector<int64_t, 5U> storageDims;
    ...
    std::vector<int64_t> at_tensor_storage_sizes;
    at_tensor_storage_sizes = at_npu::native::get_npu_storage_sizes(at_tensor);
    ...
    auto at_tensor_view = at_tensor.view(at_tensor_storage_sizes);

    auto acl_tensor = aclCreateTensor(at_tensor_storage_sizes.data(),
        at_tensor_storage_sizes.size(),
        acl_data_type,
        at_tensor_view.strides().data(),
        at_tensor_view.storage_offset(),
        format,
        storageDims.data(),
        storageDims.size(),
        const_cast<void *>(at_tensor_view.storage().data()));
    return acl_tensor;
}
...
inline aclTensor *CovertType(std::nullptr_t)
{
    return nullptr;
}
...
{
    using OpApiFunc_ = int (*)(typename std::decay<decltype(std::get<I>(params))>::type...);
    auto func = reinterpret_cast<OpApiFunc_>(opApiAddr);
    return func;
}
...
inline void AddParamToBuf(void* data_expression, size_t size_expression)
{
...
{
    AddParamToBuf(value.data(), value.size() * sizeof(bool));
}

template <typename T>
void AddParamToBuf(const T &value)
{
    AddParamToBuf(&value, sizeof(T));
}
...
template <typename... Ts>
inline void aclnn_cmd(const std::string& aclnn_api, Ts &...args)
{
    const auto getWorkspaceSizeFuncAddr = GetOpApiFuncAddr(aclnn_api + "GetWorkspaceSize");
    const auto opApiFuncAddr = GetOpApiFuncAddr(aclnn_api);
    ...
    TORCH_CHECK(getWorkspaceSizeFuncAddr != nullptr && opApiFuncAddr != nullptr,
        aclnn_api,
        " or ",
        aclnn_api + "GetWorkspaceSize not in libopapi.so, or libcust_opapi.so not found.");
    ...
    if (workspace_size != 0) {
        ...
        auto workspace_tensor = at::empty({(long)workspace_size}, options.dtype(at::kByte));
        workspace_addr = const_cast<void *>(workspace_tensor.storage().data());
    }
    auto acl_call = [converted_params, workspace_addr, workspace_size, acl_stream, executor, aclnn_api, opApiFuncAddr]() ->int {
...
#define ACLNN_CMD(aclnn_api, ...) aclnn_cmd(#aclnn_api, __VA_ARGS__)
