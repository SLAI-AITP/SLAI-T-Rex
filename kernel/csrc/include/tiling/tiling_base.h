 ...
 */
#ifndef GTSOPS_TILING_BASE_H
#define GTSOPS_TILING_BASE_H
...
#define VECTOR_INNER_ERR_REPORT_TILIING(op_name, err_msg, ...)  \
    do {                                                        \
        OP_LOGE(op_name(opname), __VA_ARGS__);                  \
    } while (0)
...
public:
    ...
    ge::graphStatus DoTiling()
    {
        try {
            ...
            OPS_LOG_E(context_, "[ERROR] DoTiling failed, exception:%s", e.what());
...
protected:
    ...
    virtual void DumpTilingInfo()
    {
        ...
        oss << "Start to dump tiling info. tilingkey:" << GetTilingKey() << ", tiling data size:"
            << bufLen << ", content:";
...
#endif // end GTSOPS_TILING_BASE_H
