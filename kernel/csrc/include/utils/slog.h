...
#define FILE_NAME "unknown" // 备选(__builtin_strrchr(__FILE__, '/') + 1)
...
#define dlog_error(moduleId, fmt, ...)                                          \
    do {                                                                          \
        DlogErrorInner(moduleId, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__); \
    } while (TMP_LOG != 0)
...
#define dlog_warn(moduleId, fmt, ...)                                               \
    do {                                                                              \
        if (CheckLogLevel(moduleId, DLOG_WARN) == 1) {                                   \
            DlogWarnInner(moduleId, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__);  \
        }                                                                               \
    } while (TMP_LOG != 0)
...
#define dlog_info(moduleId, fmt, ...)                                               \
    do {                                                                              \
        if (CheckLogLevel(moduleId, DLOG_INFO) == 1) {                                   \
            DlogInfoInner(moduleId, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__);  \
        }                                                                               \
    } while (TMP_LOG != 0)
...
#define dlog_debug(moduleId, fmt, ...)                                              \
    do {                                                                              \
        if (CheckLogLevel(moduleId, DLOG_DEBUG) == 1) {                                  \
            DlogDebugInner(moduleId, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__); \
        }                                                                               \
    } while (TMP_LOG != 0)
...
#define dlog_event(moduleId, fmt, ...)                                          \
    do {                                                                          \
        DlogEventInner(moduleId, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__); \
    } while (TMP_LOG != 0)
#endif
...
#define Dlog(moduleId, level, fmt, ...)                                                 \
    do {                                                                                  \
        if (CheckLogLevel(moduleId, level) == 1) {                                           \
            DlogInner(moduleId, level, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__);   \
        }                                                                                  \
    } while (TMP_LOG != 0)
...
#define DlogSub(moduleId, submodule, level, fmt, ...)                                                   \
    do {                                                                                                  \
        if (CheckLogLevel(moduleId, level) == 1) {                                                           \
            DlogInner(moduleId, level, "[%s:%d][%s]" fmt, FILE_NAME, __LINE__, submodule, ##__VA_ARGS__);    \
        }                                                                                                   \
    } while (TMP_LOG != 0)
#endif
/**
...
#define DlogForC(moduleId, level, fmt, ...)                                                 \
    do {                                                                                  \
        if (CheckLogLevelForC(moduleId, level) == 1) {                                           \
            DlogInnerForC(moduleId, level, "[%s:%d]" fmt, FILE_NAME, __LINE__, ##__VA_ARGS__);   \
        }                                                                                  \
    } while (TMP_LOG != 0)
...
#define DlogSubForC(moduleId, submodule, level, fmt, ...)                                                   \
    do {                                                                                                  \
        if (CheckLogLevelForC(moduleId, level) == 1) {                                                           \
            DlogInnerForC(moduleId, level, "[%s:%d][%s]" fmt, FILE_NAME, __LINE__, submodule, ##__VA_ARGS__);    \
        }                                                                                                   \
    } while (TMP_LOG != 0)
