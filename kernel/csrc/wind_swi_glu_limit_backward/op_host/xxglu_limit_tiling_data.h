 ...
 */
#ifndef GTSOPS_SWIGLU_LIMIT_BACKWARD_V2_TILING_DATA_H
#define GTSOPS_SWIGLU_LIMIT_BACKWARD_V2_TILING_DATA_H
...
namespace optiling {
BEGIN_TILING_DATA_DEF(XxGluLimitBackwardTilingData)
    ...
    TILING_DATA_FIELD_DEF(float, limit);                // gate/up clamp 阈值，0=不裁剪
    TILING_DATA_FIELD_DEF(uint32_t, hasProbs);          // 是否有 probs 输入(MoE per-token 权重)，0=无
END_TILING_DATA_DEF;

REGISTER_TILING_DATA_CLASS(WindSwiGluLimitBackward, XxGluLimitBackwardTilingData)      // 注册1：反向算子缺省结构注册

// tiling for single input tensor, the memory of the two input tenors
BEGIN_TILING_DATA_DEF(XxGluLimitBackwardSingleTilingData)
...
REGISTER_TILING_DATA_CLASS(WindSwiGluLimitBackward_1, XxGluLimitBackwardSingleTilingData)   // 注册2：反向单输入结构算子名带后缀
};

#endif // GTSOPS_SWIGLU_LIMIT_BACKWARD_V2_TILING_DATA_H
