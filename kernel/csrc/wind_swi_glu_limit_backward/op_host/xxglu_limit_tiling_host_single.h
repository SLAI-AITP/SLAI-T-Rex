 ...
 */
#ifndef OP_XXGLU_LIMIT_TILING_HOST_SINGLE_H
#define OP_XXGLU_LIMIT_TILING_HOST_SINGLE_H
...
namespace optilingLimitBackwardSingle { // 独立 namespace，避免与老 swi_glu_backward 的 optilingSingle::GluSingleTilingCalculator/Tiling4Glu ODR 冲突
...
// probs 融合再加 2 个 fp32 TBuf(probsBuf_ + probsVecBuf_)，同步 +2。
// grad_probs 不新增 TBuf：h/reduce 输出别名复用 tempLocal/probsLocal_，clamp 原地写 bF32Local。
...
private:
    ...
    mutable optiling::XxGluLimitBackwardSingleTilingData tiling_;
...
template <GLU_FLAG Glu_Flag> ge::graphStatus GluSingleTilingCalculator<Glu_Flag>::CalcTiling()
{
    ...
    if constexpr (Glu_Flag == SWIGLU) {
        // limit 占用 attr index1，highPrecision 顺移到 index2
...
#endif  // OP_XXGLU_LIMIT_TILING_HOST_SINGLE_H
