 ...
 */
#ifndef OP_SWIGLU_LIMIT_BACKWARD_V2_TILING_KERNEL_H
#define OP_SWIGLU_LIMIT_BACKWARD_V2_TILING_KERNEL_H
...
struct XxgluTilingKernel {
    ...
    // calc tiling data
    __aicore__ void GetTilingAndOffset(const XxGluLimitBackwardTilingData* tempTilingGm, uint32_t inputDTypeLen)
    {
        ...
        // 直接索引 singleCoreBlockLens 会读到数组越界/垃圾值 -> gmOffset 天文数字 -> GM 地址越界(vector core exception)。空转跳过。
        if (AscendC::GetBlockIdx() >= tempTilingGm->usedCoreNum) {
            blockLength = 0;
...
#endif // OP_SWIGLU_LIMIT_BACKWARD_V2_TILING_KERNEL_H
