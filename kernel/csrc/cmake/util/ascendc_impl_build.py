...
def {}({}, kernel_name="{}", impl_mode=""):
	...
	# add ascendc api path
    options.append("-I" + os.path.join(PYF_PATH, "../../../../../../", "ascend_compiler_api"))
    ...
    options.append("-I" + os.path.join(PYF_PATH, "../../../../../", "include"))
    options.append("-I" + os.path.join(PYF_PATH, "../../../../../", "include/tiling"))
...
COMPILE_OP_API = '''
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\\
        ...
        param_type_dynamic = {}, param_type_list = {})
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')
'''
...
def optype_snake_ex(origin_str):
    pre = 0;
    out = ''
    for it in origin_str:
        ival = ord(it)
        # letter A-Z
        if ival >= 65 and ival <= 90:
            # letter no A-Z and pre not '_' and not first letter
            if (pre < 65 or pre > 90) and pre != 95 and pre != 0:
                out += '_'
            out += it.lower()
        else:
            out += it
    ...
    def _write_impl(self: any, fd: object):
        ...
        if self.op_replay_flag:
            ...
            fd.write(COMPILE_OP_API.format(self.op_type, kern_name, self.op_type, ', '.join(self.input_name),\
                ', '.join(self.output_name), self.param_type_dynamic, self.input_type + self.output_type,
...
if __name__ == '__main__':
    ...
    op_compile_option = {}
    if len(sys.argv) > 7:
        if "flash_attention_s" in sys.argv[4]:
            data = {"compile_options":{"__ALL__": sys.argv[7].split(",")}}
            op_compile_option["FlashAttentionS"] = json.dumps(data)
        if "incre_flash_attention_s" in sys.argv[4]:
            data = {"compile_options":{"__ALL__": sys.argv[7].split(",")}}
            op_compile_option["IncreFlashAttentionS"] = json.dumps(data)
    write_scripts(cfgfile=sys.argv[1], cfgs=rep_cfg, dirs=cfg_dir, op_compile_option=op_compile_option)
