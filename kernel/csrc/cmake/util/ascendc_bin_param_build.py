    ...
    def gen_input_json(self: any):
        ...
        for i in range(0, count):
            ...
            for idx in range(0, len(self.input_name)):
                ...
                para = {'name': self.input_name[idx], 'index': idx, 'dtype': idtypes[i], 'format': ifmts[i],
                        'paramType': itype, 'shape': [-2], 'format_match_mode': 'FormatAgnostic'}
                inputs.append([para]) if itype == 'dynamic' else inputs.append(para)
            for idx in range(0, len(self.output_name)):
                ...
                para = {'name': self.output_name[idx], 'index': idx, 'dtype': odtypes[i], 'format': ofmts[i],
                        'paramType': otype, 'shape': [-2], 'format_match_mode': 'FormatAgnostic'}
                outputs.append([para]) if otype == 'dynamic' else outputs.append(para)
            ...
            op_node['deterministic'] = 'ignore'
            if len(attrs) > 0: op_node['attrs'] = attrs
            param = {}
            ...
            bin_file = self.op_type + '_' + md5sum + "_high_performance"
