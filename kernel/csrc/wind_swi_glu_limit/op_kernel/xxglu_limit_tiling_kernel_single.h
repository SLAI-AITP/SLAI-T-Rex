 ...
 */
#ifndef OP_SWIGLU_LIMIT_V2_TILING_KERNEL_SINGLE_H   // [Add by GTS-AILab] [feature GTSOps]
#define OP_SWIGLU_LIMIT_V2_TILING_KERNEL_SINGLE_H   // [Add by GTS-AILab] [feature GTSOps]
...
struct XxGluSingleTileOffsetParam {
    ...
    uint64_t probsGmOffset; // 拼接的vector，第er个vector gm上的偏移
...
struct XxgluSingleTilingKernel {
    ...
    // calc tiling data
    __aicore__ void GetTilingAndOffset(const XxGluLimitSingleTilingData* tempTilingGm, uint32_t inputDTypeLen)
    {
    ...
    __aicore__ inline void CaclOneTileOffsetParam(uint64_t gmRowOffset, uint64_t colIdx)
    {
        ...
        offsetParam.probsGmOffset = gmRowOffset;
...
#define XXGLU_SINGLE_PROBS_PROCESS_TILE(offsetParam, copyParam, calLen, curRowLen) \
do { \
    CopyIn(offsetParam, copyParam); \
    uint32_t curColLen = (calLen) / curRowLen; \
    this->Compute(calLen, curRowLen, curColLen, offsetParam.probsGmOffset); \
    CopyOut(offsetParam, copyParam); \
} while (0)

#define XXGLU_SINGLE_PROBS_PROCESS(kernelTiling) \
do {                                       \
    ...
    for (uint64_t tileIdx = AscendC::GetBlockIdx(); tileIdx < kernelTiling.totalTileNum; tileIdx += blockNum) { \
        ...
        XXGLU_SINGLE_PROBS_PROCESS_TILE(kernelTiling.offsetParam, *(kernelTiling.curTileCopyParam), kernelTiling.curCalLen, kernelTiling.curRowLen); \
    } \
} while (0)

#define XXGLU_SINGLE_PROBS_PROCESS_TILE_NON32BALIGNED(offsetParam, copyParam, calLen, curRowLen) \
do {                                              \
...
#define XXGLU_SINGLE_PROBS_PROCESS_NON32BALIGNED(kernelTiling) \
do {                                       \
    uint64_t blockNum = GetBlockNum();       \
    for (uint64_t tileIdx = AscendC::GetBlockIdx(); tileIdx < kernelTiling.totalTileNum; tileIdx += blockNum) {    \
        kernelTiling.CaclOneTileParam(tileIdx);                                                             \
        XXGLU_SINGLE_PROBS_PROCESS_TILE_NON32BALIGNED(kernelTiling.offsetParam,                                   \
            *(kernelTiling.curTileCopyParam), kernelTiling.curCalLen, kernelTiling.curRowLen);                                      \
    } \
} while (0)
...
#endif  // OP_SWIGLU_LIMIT_V2_TILING_KERNEL_SINGLE_H   // [Add by GTS-AILab] [feature GTSOps]
