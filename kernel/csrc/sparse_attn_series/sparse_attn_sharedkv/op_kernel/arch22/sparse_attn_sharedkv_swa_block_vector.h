...
__aicore__ inline void SWAVectorBlock<SAST>::ProcessLse(const RunInfo &info, const MSplitInfo &mSplitInfo)
{
    ...
    uint64_t _lseCap = (uint64_t)constInfo.batchSize * constInfo.qSeqSize * constInfo.kvHeadNum * constInfo.gSize;
    if (lseOffset + mSplitInfo.vecDealM > _lseCap) return;   // phantom 预取块，跳过 lse 写
...
__aicore__ inline void SWAVectorBlock<SAST>::Bmm2DataCopyOutTrans(const RunInfo &info, LocalTensor<OUT_T> &attenOutUb,
                                                                  ...
                                                                  uint32_t columnCount, uint32_t actualColumnCount)
{
    uint64_t _aoBase = info.attenOutOffset + (uint64_t)wsMStart * actualColumnCount;
    uint64_t _aoSpan = (uint64_t)dealRowCount * actualColumnCount;
    uint64_t _aoCap  = (uint64_t)constInfo.batchSize * constInfo.qSeqSize * constInfo.qHeadNum * constInfo.headDim;
    if (_aoBase + _aoSpan > _aoCap) return;   // phantom 预取块，不写回
    DataCopyExtParams dataCopyParams;
    dataCopyParams.blockCount = dealRowCount;
