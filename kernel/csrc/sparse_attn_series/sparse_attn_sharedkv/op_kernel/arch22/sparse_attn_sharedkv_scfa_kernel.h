...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::GetSparseActualSeqLen()
{
    ...
    // SCFA batching: cmp is SHARED across the BQ queries using the FIRST query's causal count
    // (s1StartIdx). Gather-fewer is the safe approximation: a later query in the batch may miss
    // <=1 newest cmp block (small error), but no query ever attends a future cmp block.
    // BQ=1 -> s1StartIdx==s1EndIdx -> identical to baseline.
    int32_t thresHold = (tempLoopInfo.cmpMaskRight + tempLoopInfo.s1StartIdx + 1) / constInfo.cmpRatio;
    ...
    TPipe *tPipe)
{
    ...
    offset += GetBlockNum() * 512 * 512 * MERGE_CACHE_GM_BUF_NUM * sizeof(KV_T);
                                                                ...
                                                                uint32_t s2LoopIdx, RunInfo &info)
{
    ...
    if (s2LoopIdx < tempLoopInfo.oriLoopTimes) {
        ...
        info.cmpS2IdLimit = (tempLoopInfo.cmpMaskRight + tempLoopInfo.s1StartIdx + 1) / constInfo.cmpRatio;
    } else {
        info.isOri = false;
...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::ProcessBalance()
{
    ...
    for (uint32_t bN2LoopIdx = constInfo.bN2Start; bN2LoopIdx < constInfo.bN2End; bN2LoopIdx++) {
        ...
        for (uint32_t gS1LoopIdx = constInfo.gS1Start; gS1LoopIdx < gS1LoopEnd; gS1LoopIdx++) {
            ...
            // 此处均为闭区间
            // ori window is the UNION over the BQ queries in this M-tile: right edge from the
            // LAST query (s1EndIdx), left edge from the FIRST query (s1StartIdx). Each query is
            // then masked to its own window per-row in the vector (DealBmm1ResBaseBlock).
            // BQ=1 -> s1StartIdx==s1EndIdx -> identical to baseline.
