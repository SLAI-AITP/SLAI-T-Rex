...
__aicore__ inline void SWACubeBlock<SAST>::CopyInMm1AToL1(LocalTensor<KV_T> &l1Tensor, const RunInfo &info,
                                                          ...
                                                          uint32_t headOffset)
{
    uint64_t _mTotal = (uint64_t)constInfo.qSeqSize * constInfo.qHeadNum;   // M 轴有效行数
    uint64_t _mRow   = info.tensorAOffset / constInfo.headDim + mSeqIdx;
    if (_mRow + mSizeAct > _mTotal) _mRow = (_mTotal > mSizeAct) ? (_mTotal - mSizeAct) : 0;
    auto srcGm = queryGm[_mRow * constInfo.headDim + headOffset];
        ...
        for (uint32_t kL1 = 0; kL1 < kL1Loops; kL1++) {
            ...
            if (info.isOri) {
                if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                    ...
                    uint32_t _rows = nL1Size;
                    uint64_t curS2 = (uint64_t)info.s2Idx * constInfo.s2BaseSize + info.s2StartPoint +
                        (uint64_t)nL1 * N_SPLIT_SIZE;
                    if (curS2 + _rows > constInfo.kvSeqSize)
                        curS2 = (constInfo.kvSeqSize > _rows) ? (constInfo.kvSeqSize - _rows) : 0;
                ...
                if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                    ...
                    if (oriSizeCur > 0) {
                        ...
                        uint64_t curS2 = (uint64_t)info.s2Idx * constInfo.s2BaseSize + info.s2StartPoint +
                            nL1 * N_SPLIT_SIZE;
                        if (curS2 + nd2nzPara.nValue > constInfo.kvSeqSize)  // [FIX] clamp phantom OOB (keep DataCopy)
                            curS2 = (constInfo.kvSeqSize > nd2nzPara.nValue) ? (constInfo.kvSeqSize - nd2nzPara.nValue) : 0;
                        uint64_t offset = (uint64_t)info.bIdx * batchStride + (uint64_t)curS2 * seqStride +
                            (uint64_t)info.n2Idx * headStride + kL1 * D_SPLIT_SIZE;
                        DataCopy(bL1Tensor, oriKvGm[offset], nd2nzPara);
                    }
                    if (cmpSizeCur > 0) {
                        nd2nzPara.nValue = cmpSizeCur;                       // [FIX] cmp 实际行数(原误用 nL1Size)
                        uint32_t cmpSeq = constInfo.kvSeqSize / constInfo.cmpRatio;
                        uint32_t batchStride = cmpSeq * seqStride;
                        uint32_t cmpMixsizeCur = N_SPLIT_SIZE - info.actualSingleProcessSInnerOriSize % N_SPLIT_SIZE;
                        uint32_t oriOnlyLoopTimes = info.actualSingleProcessSInnerOriSize / N_SPLIT_SIZE;
                        uint32_t cmpLoopTimes = nL1 - oriOnlyLoopTimes;
                        uint64_t curS2 = (cmpLoopTimes == 0) ? 0 : (cmpMixsizeCur + (cmpLoopTimes - 1) * N_SPLIT_SIZE);
                        if (curS2 >= cmpSeq) curS2 = (cmpSeq > 0) ? (cmpSeq - 1) : 0;   // [FIX] clamp into cmp_kv
                        if (curS2 + nd2nzPara.nValue > cmpSeq) nd2nzPara.nValue = cmpSeq - curS2;
                        uint64_t offset = (uint64_t)info.bIdx * batchStride + curS2 * seqStride +
                            (uint64_t)info.n2Idx * headStride + kL1 * D_SPLIT_SIZE;
            ...
            for (uint32_t kL1 = kOffset; kL1 < kL0Loops + kOffset; kL1++) { // 128 循环搬pa
                ...
                if (info.isOri) {
                    if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                        ...
                        Nd2NzParams nd2nzPara;
                        nd2nzPara.ndNum = 1;
                        nd2nzPara.nValue = kL0Size;      // 行数(沿 S2 收缩轴，已被尾块逻辑 clamp)
                        nd2nzPara.dValue = nL1Size;      // 列数(沿 D)
                        nd2nzPara.srcDValue = constInfo.headDim;
                    ...
                    if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                        ...
                        if (oriSizeCur > 0) {
                            ...
                            uint64_t curS2 = (uint64_t)info.s2Idx * constInfo.s2BaseSize + info.s2StartPoint +
                                kL1 * K_L0_SPLIT_SIZE;
                            if (curS2 + nd2nzPara.nValue > constInfo.kvSeqSize)   // [FIX] clamp phantom OOB
                                curS2 = (constInfo.kvSeqSize > nd2nzPara.nValue) ? (constInfo.kvSeqSize - nd2nzPara.nValue) : 0;
                            uint64_t offset = (uint64_t)info.bIdx * batchStride + (uint64_t)curS2 * seqStride +
                                (uint64_t)info.n2Idx * headStride + nL1 * N_SPLIT_SIZE;
                            DataCopy(subvTensor, oriKvGm[offset], nd2nzPara);
