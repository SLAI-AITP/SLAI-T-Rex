...
public:
    ...
    __aicore__ inline void ApplyOriBandMask(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                            LocalTensor<T> &mmResUb, uint32_t startRow, uint32_t dealRowCount,
...
template <typename SAST>
__aicore__ inline void SASVectorBlock<SAST>::ApplyOriBandMask(const RunInfo &info, const MSplitInfo &mSplitInfo,
                                                              LocalTensor<T> &mmResUb, uint32_t startRow,
                                                              uint32_t dealRowCount, uint32_t columnCount)
{
    // SCFA query-batching: the loaded ori columns are the UNION window over the BQ queries in this
    // M-tile. Mask each query's rows down to its OWN window [q-winLeft, q+winRight] (in gathered-
    // column space, query qLocal's valid columns are globalC in [qLocal, qLocal+winSpan]). No-op
    // when actMBaseSize==gSize (BQ=1 baseline). cmp splits are shared (gathered on the first
    // query) so they are never masked here.
    if (!info.isOri || info.actMBaseSize <= constInfo.gSize) {
        return;
    }
    int32_t realCols = static_cast<int32_t>(info.actualSingleProcessSInnerSize);
    if (realCols <= 0) {
        return;
    }
    uint32_t bq = info.actMBaseSize / constInfo.gSize;
    int32_t splitBase = static_cast<int32_t>(info.s2Idx) * static_cast<int32_t>(constInfo.s2BaseSize);
    int32_t winSpan = static_cast<int32_t>(constInfo.oriWinLeft) + static_cast<int32_t>(constInfo.oriWinRight);
    int32_t absRowStart = static_cast<int32_t>(mSplitInfo.nBufferStartM + mSplitInfo.vecStartM + startRow);
    int32_t absRowEnd = absRowStart + static_cast<int32_t>(dealRowCount);
    uint8_t repStride = static_cast<uint8_t>(columnCount / FP32_BLOCK_ELEMENT_NUM);
    const T negVal = static_cast<T>(-1e30);
    for (uint32_t qLocal = 0; qLocal < bq; qLocal++) {
        int32_t qRowLo = static_cast<int32_t>(qLocal * constInfo.gSize);
        int32_t qRowHi = qRowLo + static_cast<int32_t>(constInfo.gSize);
        qRowLo = (qRowLo > absRowStart) ? qRowLo : absRowStart;
        qRowHi = (qRowHi < absRowEnd) ? qRowHi : absRowEnd;
        if (qRowLo >= qRowHi) {
            continue;
        }
        uint32_t locRow = static_cast<uint32_t>(qRowLo - absRowStart);
        uint8_t rowCnt = static_cast<uint8_t>(qRowHi - qRowLo);
        // left-invalid columns c in [0, qLocal - splitBase)
        int32_t leftEndC = static_cast<int32_t>(qLocal) - splitBase;
        if (leftEndC > realCols) { leftEndC = realCols; }
        for (int32_t c0 = 0; c0 < leftEndC; c0 += 64) {
            uint64_t mcnt = static_cast<uint64_t>((leftEndC - c0 < 64) ? (leftEndC - c0) : 64);
            Duplicate(mmResUb[locRow * columnCount + c0], negVal, mcnt, rowCnt, 1, repStride);
        }
        // right-invalid columns c in [qLocal + winSpan - splitBase + 1, realCols)
        int32_t rightStartC = static_cast<int32_t>(qLocal) + winSpan - splitBase + 1;
        if (rightStartC < 0) { rightStartC = 0; }
        for (int32_t c0 = rightStartC; c0 < realCols; c0 += 64) {
            uint64_t mcnt = static_cast<uint64_t>((realCols - c0 < 64) ? (realCols - c0) : 64);
            Duplicate(mmResUb[locRow * columnCount + c0], negVal, mcnt, rowCnt, 1, repStride);
        }
    }
}
                                                                  ...
                                                                  uint32_t columnCount, uint32_t loopId)
{
    ...
    ApplyOriBandMask(info, mSplitInfo, mmResUb, startRow, dealRowCount, columnCount); // no-op for BQ=1
