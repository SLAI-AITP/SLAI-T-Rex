...
void SparseAttnSharedkvTiling::SplitBalanced(SASTilingInfo *tilingInfo)
{
    ...
    // [OPT] mBaseSize sweep target for SWA/CFA. With gSize=64 and M_SPLIT_SIZE=128:
    //   64  -> M=64  : half-fills the cube M tile, 4096 blocks (fixpipe/MTE2/scalar bound) = baseline 881us
    //   128 -> M=128 : fully fills the cube M tile, 2048 blocks, mL1Loops still 1  [recommended]
    //   256 -> M=256 : mL1Loops=2 -> cube/vector sync mismatch -> DEADLOCK (do not use)
    // cmp/mix OOB is now clamped, so 128 no longer faults. SCFA keeps gSize unchanged.
    constexpr uint32_t SAS_M_BASE_TARGET = 64;
    // [OPT] SCFA query-batching: process SAS_SCFA_BQ consecutive queries per cube M-tile
    // (M = gSize*BQ) to fill the 128-row cube tile (halves block count). BQ=1 -> M=gSize
    // (byte-identical baseline). Capped at 128 (=M_SPLIT_SIZE) to keep mL1Loops==1 (M=256
    // deadlocks). MUST match SAS_SCFA_BQ in sparse_attn_sharedkv_metadata_aicpu.cpp.
    // NOTE: BQ=2 is numerically CORRECT (precision passes) but regressed the forward ~7.5x:
    // doubling the M-tile cost drives the metadata cost-scheduler into heavy cross-core
    // S2-splitting -> cube/vector overlap collapses (cube_util 67%->7.75%). Left at 1 (baseline)
    // until the metadata core/S2 split is tuned for mBaseSize>gSize. Must match metadata aicpu.
    constexpr uint32_t SAS_SCFA_BQ = 1;
    uint32_t scfaM = tilingInfo->gSize * SAS_SCFA_BQ;
    if (scfaM > 128U) {
        scfaM = tilingInfo->gSize; // cap: keep M<=128
    }
    uint32_t mtgt = (SAS_M_BASE_TARGET < tilingInfo->gSize) ? tilingInfo->gSize : SAS_M_BASE_TARGET;
    mBaseSize_ = (tilingInfo->perfMode == SASTemplateMode::SCFA_TEMPLATE_MODE)
        ? scfaM
        : (mtgt / tilingInfo->gSize) * tilingInfo->gSize;
