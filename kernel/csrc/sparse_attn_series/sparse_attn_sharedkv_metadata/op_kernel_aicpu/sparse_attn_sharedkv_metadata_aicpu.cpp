...
bool SparseAttnSharedkvMetadataCpuKernel::ParamsInit()
{
    ...
    // SCFA query-batching: M-tile spans SAS_SCFA_BQ queries. MUST match SAS_SCFA_BQ in
    // sparse_attn_sharedkv_tiling.cpp (the per-core split here must use the same mBaseSize as
    // the forward kernel). BQ=1 -> groupSize_ (baseline). Capped at 128.
    // Kept at 1 (baseline): BQ=2 is correct but regresses forward perf via the cost scheduler
    // (see sparse_attn_sharedkv_tiling.cpp). Must match SAS_SCFA_BQ there.
    constexpr uint32_t SAS_SCFA_BQ = 1U;
    uint32_t scfaM = groupSize_ * SAS_SCFA_BQ;
    if (scfaM > 128U) {
        scfaM = groupSize_;
    }
    mBaseSize_ = isSCFA ? scfaM : groupSize_;
    s2BaseSize_ = 512U;
    return true;
}
