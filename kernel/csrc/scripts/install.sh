...
if [ -n "${INSTALL_PATH}" ]; then
    ...
    if [ ! -d "${INSTALL_PATH}" ]; then
        mkdir ${INSTALL_PATH} -m 750 >> /dev/null 2>&1
        if [ $? -ne 0 ]; then
    ...
    if [ ! -d "${targetdir}/${vendordir}" ]; then
        mkdir -p ${targetdir}/${vendordir} -m 750 >> /dev/null 2>&1
        if [ $? -ne 0 ]; then
...
undefined_count=$(ldd -r ${sourcedir}/${vendordir}/op_impl/ai_core/tbe/op_tiling/liboptiling.so | grep -c "not found\|undefined symbol")
if [ ${undefined_count} -ne 0 ]; then
  log "[ERROR] ldd ${sourcedir}/${vendordir}/op_impl/ai_core/tbe/op_tiling/liboptiling.so failed"
  ldd -r ${sourcedir}/${vendordir}/op_impl/ai_core/tbe/op_tiling/liboptiling.so | grep "not found\|undefined symbol"
  log "[ERROR] The library does not match, please check."
  exit 1
fi
...
upgrade()
{
    ...
    if [ ! -d "${targetdir}/${vendordir}/$1" ];then
        ...
        mkdir -p ${targetdir}/${vendordir}/$1 -m 750
        if [ $? -ne 0 ];then
    ...
    if [ -d "${targetdir}/${vendordir}/$1/" ]; then
        chmod 750 -R "${targetdir}/${vendordir}/$1/" >/dev/null 2>&1
    fi
...
upgrade_proto()
{
    ...
    if [ ! -d "${targetdir}/${vendordir}/framework/caffe" ];then
        ...
        mkdir -p ${targetdir}/${vendordir}/framework/caffe -m 750
        if [ $? -ne 0 ];then
    ...
    chmod 750 -R "${targetdir}/${vendordir}/framework/caffe/" >/dev/null 2>&1
...
if [ ! -d "${targetdir}/vendors" ];then
        ...
        mkdir -p ${targetdir}/vendors -m 750
        if [ $? -ne 0 ];then
            log "[ERROR] create ${targetdir}/vendors failed"
            return 1
        fi
fi
chmod 750 ${targetdir}/vendors
...
cp -rf ${sourcedir}/${vendordir}/* ${targetdir}/${vendordir}/
if [ $? -ne 0 ]; then
    log "[ERROR] copy vendor files failed"
    exit 1
fi
...
if [ -n "${INSTALL_PATH}" ] && [ -d "${INSTALL_PATH}" ]; then
    ...
    if [ ! -d "${bin_path}" ]; then
        mkdir -p ${bin_path} -m 750 >> /dev/null 2>&1
        if [ $? -ne 0 ]; then
...
find ${targetdir}/${vendordir}/ -type f | xargs chmod g-w
find ${targetdir}/${vendordir}/ -type f | xargs chmod o-rwx
find ${targetdir}/${vendordir}/ -type f -name "*.json" -o -name "*.h" -o -name "*.o" -o -name "*.info" | xargs chmod 440
find ${targetdir}/${vendordir}/ -type f -name "*.so" | xargs chmod 550
